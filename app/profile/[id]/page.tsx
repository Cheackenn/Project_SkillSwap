  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: profileData, error: profileError } = await supabase
        .from('profiles')
        .select('id, first_name, last_name, username, bio, avatar_url, role, is_verified, skills_to_teach')
        .eq('id', userId)
        .single();
      ...
      // Get likes, comments, ratings
      const { data: likesRows } = await supabase.from('post_likes').select('post_id').in('post_id', postIds);
      ...
      const { data: commentRows } = await supabase.from('post_comments').select('post_id').in('post_id', postIds);
    ...
    return (
      <div className="min-h-screen bg-[#1a2c36] pb-24">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="p-6 text-center text-red-400">{error || 'Profile not found'}</div>
        </div>
        <BottomNav />
      </div>
    );
    ...
    <div className="min-h-screen bg-[#1a2c36] pb-24">
      <div className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex gap-8 w-full justify-center pt-4 border-t border-[#3a4f5a]">
              <div className="text-center">
                <p className="text-2xl font-bold text-[#5fa4c3]">{userPosts.length}</p>
            ...
            {isOwnProfile ? (
              ...
              <button
                ...
                className="w-full px-4 py-2 bg-[#5fa4c3] text-white rounded-full font-medium hover:bg-[#4a8fb5] transition-colors text-center flex items-center justify-center gap-2"
              >
        ...
        <div className="p-4 space-y-4">
          ...
          {userPosts.length > 0 ? (
            userPosts.map((post) => <PostCard key={post.id} post={post} onDeletePost={() => {}} />)
          ) : (
